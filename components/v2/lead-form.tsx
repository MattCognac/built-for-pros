      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="grid gap-2">
          ...
          <input
            ...
            autoComplete="name"
            required
            className="min-h-12 rounded-xl border border-white/[0.1] bg-[var(--v2-canvas)] px-4 text-base text-zinc-50 outline-none transition placeholder:text-zinc-600 focus:border-[color:var(--brand)] focus:ring-2 focus:ring-[color:var(--brand)]/25"
      ...
      {message ? (
        <p
          className={`text-sm leading-6 ${
            status === "error" ? "text-rose-400" : "text-emerald-400/95"
          }`}
          aria-live="polite"
        >
          {message}
